...
clean:
	...
	@$(RM) bin/*/*/appview lib/*/*/libappview.so
	@$(RM) -rf newdir testtempdir1 testtempdir2 coverage go .bash_history .cache .viminfo appview.log *.gcda
...
# include the CLI targets with "cli" prefix plus "appview" shortcut
cli%:
	@$(MAKE) -C cli $(@:cli%=%)
appview:
	@$(MAKE) -C cli $@
...
build: require-docker require-qemu-binfmt
	...
	@docker run --rm $(if $(CI),,-it) \
		-v $(shell pwd):/home/builder/appview \
		-u $(shell id -u):$(shell id -g) \
		--privileged \
		--platform $(ARCH) \
		$(if $(CI),,--name appview-builder-$(DIST)-$(ARCH)) \
...
run:
	@echo Running the AppView $(DIST)/$(ARCH) Builder Container
...
exec: CMD ?= /bin/bash
exec:
	@[ -n "$(shell docker ps -q -f "name=appview-builder-$(DIST)-$(ARCH)")" ] || \
		{ echo >&2 "error: appview-builder-$(DIST)-$(ARCH) not running"; exit 1; }
	@echo "Exec'ing into the AppView $(DIST)/$(ARCH) Builder Container"
	@docker exec -it $(shell docker ps -q -f "name=appview-builder-$(DIST)-$(ARCH)") $(CMD)
...
builder: require-docker-buildx-builder
	...
	@docker buildx build \
		...
		--label "org.opencontainers.image.description=AppView $(ARCH) Builder ($(PLATFORM_$(ARCH)))" \
		$(if $(NOLOAD),,--load) \
...
docs-generate: require-docker-buildx-builder
	...
	@docker buildx build \
		...
		.
	@echo Running the AppView docs generator
	@docker run \
		-v $(shell pwd):/home/node/appview \
		--rm $(TAG)

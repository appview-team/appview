...
// var defaultAppViewConfigYaml string

// func init() {
// 	defaultAppViewConfigYaml = `metric:
//   enable: true
//   format:
//     type: ndjson
//     verbosity: 4
...
func TestSetDefault(t *testing.T) {
	...
	testYaml := testDefaultAppViewConfigYaml("/foo", 4)
...
func TestConfigFromRunOpts(t *testing.T) {
	...
	// unix
	c.MetricsDest = "unix:///tmp/file_socket"
	err = c.configFromRunOpts()
	assert.NoError(t, err)
	assert.Equal(t, "unix", c.sc.Metric.Transport.TransportType)
	assert.Equal(t, "/tmp/file_socket", c.sc.Metric.Transport.Path)

	c.MetricsDest = "unix://@abstract_socket"
	err = c.configFromRunOpts()
	assert.NoError(t, err)
	assert.Equal(t, "unix", c.sc.Metric.Transport.TransportType)
	...
	err = c.configFromRunOpts()
	assert.NoError(t, err)
	assert.Equal(t, c.sc.Libappview.Log.Level, "debug")
}

func TestWriteAppViewConfig(t *testing.T) {
	...
	// assert.NoError(t, err)
	// f.Close()
	// err = WriteAppViewConfig(sc, ".cantwrite")
	// assert.Error(t, err, "operational not permitted")
	// os.Remove(".cantwrite")

	testYaml := testDefaultAppViewConfigYaml("/foo", 4)
	err = opt.WriteAppViewConfig(".appview.yml", 0644)
	assert.NoError(t, err)
	yamlBytes, err := ioutil.ReadFile(".appview.yml")
	assert.Equal(t, testYaml, string(yamlBytes))
	os.Remove(".appview.yml")
}

func TestAppViewLogRegex(t *testing.T) {
	tests := []struct {
		Test    string
		Matches bool
	}{
	...
	re := regexp.MustCompile(appviewLogRegex())

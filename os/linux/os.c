#define _GNU_SOURCE
#include <sys/param.h>
#include <sys/types.h>
#include <grp.h>
#include <pwd.h>
#include <time.h>
#include "os.h"
#include "../../src/appviewstdlib.h"
#include "../../src/dbg.h"
#include "../../src/fn.h"
#include "../../src/appviewtypes.h"

// want to put this list in an obvious place
//static char thread_delay_list[] = "chrome:nacl_helper";

struct sigUserStruct {
    struct sigaction appAction; // Application signal action structure
    timer_t timerId;            // Signal timer ID
    bool appviewHandlerActive;    // State of AppView Signal Handler
};

/*
* Distinguish the SIGUSR2 generated by the AppView library
*/
#define SIG_ORIGIN_APPVIEW_ID (123456789)

static struct sigUserStruct g_sigUsrState = {
    .timerId = 0,
    .appAction = {{0}},
    .appviewHandlerActive = FALSE,
};

static int
sendNL(int sd, ino_t node)
{
    struct sockaddr_nl nladdr = {
        .nl_family = AF_NETLINK
    };

    struct {
        struct nlmsghdr nlh;
        struct unix_diag_req udr;
    } req = {
        .nlh = {
            .nlmsg_len = sizeof(req),
            .nlmsg_type = SOCK_DIAG_BY_FAMILY,
            .nlmsg_flags = NLM_F_REQUEST
        },
        .udr = {
            .sdiag_family = AF_UNIX,
            .sdiag_protocol = 0,
            .pad = 0,
            .udiag_states = -1,
            .udiag_ino = node,
            .udiag_cookie[0] = -1,
            .udiag_cookie[1] = -1,
            .udiag_show = UDIAG_SHOW_PEER
        }
    }; // reminder cookies must be -1 in order for a single request to work

    struct iovec iov = {
        .iov_base = &req,
        .iov_len = sizeof(req)
    };

    struct msghdr msg = {
        .msg_name = (void *) &nladdr,
        .msg_namelen = sizeof(nladdr),
        .msg_iov = &iov,
        .msg_iovlen = 1
    };

    // should we check for a partial send?
    if (appview_sendmsg(sd, &msg, 0) < 0) {
        appviewLog(LOG_LEVEL, "fd:%d ERROR:sendNL:sendmsg", sd);
        return -1;
    }

    return 0;
}

static ino_t
getNL(int sd)
{
    int rc;
    char buf[sizeof(struct nlmsghdr) + (sizeof(long) * 4)];
    struct unix_diag_msg *diag;
    struct rtattr *attr;
    struct sockaddr_nl nladdr = {
        .nl_family = AF_NETLINK
    };

    struct iovec iov = {
        .iov_base = buf,
        .iov_len = sizeof(buf)
    };

    struct msghdr msg = {
        .msg_name = (void *) &nladdr,
        .msg_namelen = sizeof(nladdr),
        .msg_iov = &iov,
        .msg_iovlen = 1
    };

    if ((rc = appview_recvmsg(sd, &msg, 0)) <= 0) {
        appviewLog(LOG_LEVEL, "fd:%d ERROR:getNL:recvmsg", sd);
        return (ino_t)-1;
    }

    const struct nlmsghdr *nlhdr = (struct nlmsghdr *)buf;

    if (!NLMSG_OK(nlhdr, rc)) {
        appviewLog(LOG_LEVEL, "fd:%d ERROR:getNL:!NLMSG_OK", sd);
        return (ino_t)-1;
    }

    for (; NLMSG_OK(nlhdr, rc); nlhdr = NLMSG_NEXT(nlhdr, rc)) {
        if (nlhdr->nlmsg_type == NLMSG_DONE) {
            appviewLog(LOG_LEVEL, "fd:%d ERROR:getNL:no message", sd);
            return (ino_t)-1;
        }

        if (nlhdr->nlmsg_type == NLMSG_ERROR) {
            const struct nlmsgerr *err = NLMSG_DATA(nlhdr);

            if (nlhdr->nlmsg_len < NLMSG_LENGTH(sizeof(*err))) {
                appviewLog(LOG_LEVEL, "fd:%d ERROR:getNL:message error", sd);
            } else {
                appviewLog(LOG_LEVEL, "fd:%d ERROR:getNL:message errno %d", sd, -err->error);
            }

            return (ino_t)-1;
        }

        if (nlhdr->nlmsg_type != SOCK_DIAG_BY_FAMILY) {
            appviewLog(LOG_LEVEL, "fd:%d ERROR:getNL:unexpected nlmsg_type", sd);
            return (ino_t)-1;
        }

        if ((diag = NLMSG_DATA(nlhdr)) != NULL) {
            if (nlhdr->nlmsg_len < NLMSG_LENGTH(sizeof(*diag))) {
                appviewLog(LOG_LEVEL, "fd:%d ERROR:getNL:short response", sd);
                return (ino_t)-1;
            }

            if (diag->udiag_family != AF_UNIX) {
                appviewLog(LOG_LEVEL, "fd:%d ERROR:getNL:unexpected family", sd);
                return (ino_t)-1;
            }

            attr = (struct rtattr *) (diag + 1);
            if (attr->rta_type == UNIX_DIAG_PEER) {
                if (RTA_PAYLOAD(attr) >= sizeof(unsigned int)) {
                    return (ino_t)*(unsigned int *) RTA_DATA(attr);
                }
            }
        }
    }
    return (ino_t)-1;
}

static uint64_t
getProcVal(char *srcbuf, const char *tag)
{
    char *entry, *last, *buf;
    uint64_t val = -1;
    const char delim[] = ":";

    if (!srcbuf) return -1;
    buf = appview_strdup(srcbuf);

    entry = appview_strtok_r(buf, delim, &last);
    while (1) {
        if ((entry = appview_strtok_r(NULL, delim, &last)) == NULL) {
            break;
        }

        if (appview_strcasestr((const char *)entry, tag) != NULL) {
            // The next token should be what we want
            if ((entry = appview_strtok_r(NULL, delim, &last)) != NULL) {
                if ((val = (uint64_t)appview_strtoll(entry, NULL, 0)) == (long long)0) {
                    val = (uint64_t)-1;
                }
                break;
            }
        }
    }

    if (buf) appview_free(buf);
    return val;
}


int
osUnixSockPeer(ino_t lnode)
{
    int nsd = appview_socket(AF_NETLINK, SOCK_RAW, NETLINK_SOCK_DIAG);
    ino_t rnode;

    if (nsd == -1) return -1;

    if (sendNL(nsd, lnode) == -1) {
        appview_close(nsd);
        return -1;
    }

    rnode = getNL(nsd);
    appview_close(nsd);
    return rnode;
}

int
osGetExePath(pid_t pid, char **path)
{
    if (!path) return -1;
    char *buf = *path;
    char pidpath[PATH_MAX];

    if (!(buf = appview_calloc(1, PATH_MAX))) {
        appviewLogError("ERROR:appview_calloc in osGetExePath");
        return -1;
    }

    appview_snprintf(pidpath, PATH_MAX, "/proc/%d/exe", pid);

    if (appview_readlink(pidpath, buf, PATH_MAX - 1) == -1) {
        appviewLogError("osGetExePath: can't get path to pid %d exe", pid);
        appview_free(buf);
        return -1;
    }

    *path = buf;
    return 0;
}

int
osGetProcname(char *pname, int len)
{
    if (program_invocation_short_name != NULL) {
        appview_strncpy(pname, program_invocation_short_name, len);
    } else {
        char *ppath = NULL;

        if (osGetExePath(appview_getpid(), &ppath) != -1) {
            appview_strncpy(pname, appview_basename(ppath), len);
            if (ppath) appview_free(ppath);
        } else {
            return -1;
        }
    }
    return 0;
}

int
osGetProcUidGid(pid_t pid, uid_t *uid, gid_t *gid)
{
    char path[PATH_MAX] = {0};
    char buffer[4096];
    uid_t eUid = -1;
    gid_t eGid = -1;

    if (appview_snprintf(path, sizeof(path), "/proc/%d/status", pid) < 0) return -1;

    FILE *fstream = appview_fopen(path, "r");
    if (fstream == NULL) {
        return -1;
    }

    while (appview_fgets(buffer, sizeof(buffer), fstream)) {
        const char delimiters[] = ": \t";
        if (appview_strstr(buffer, "Uid:")) {
            char *entry, *last;
            // Skip Uid string
            entry = appview_strtok_r(buffer, delimiters, &last);
            // Get real Uid value
            entry = appview_strtok_r(NULL, delimiters, &last);
            eUid = appview_atoi(entry);
        }

        if (appview_strstr(buffer, "Gid:")) {
            char *entry, *last;
            // Skip Gid string
            entry = appview_strtok_r(buffer, delimiters, &last);
            // Get real Gid value
            entry = appview_strtok_r(NULL, delimiters, &last);
            eGid = appview_atoi(entry);
        }
    }

    appview_fclose(fstream);

    if (eUid != -1 && eGid != -1) {
        *uid = eUid;
        *gid = eGid;
        return 0;
    }

    return -1;
}

int
osGetProcMemory(pid_t pid)
{
    int fd;
    long result;
    char *start, *entry, *last;
    const char delim[] = ":";
    char buf[2048];

    appview_snprintf(buf, sizeof(buf), "/proc/%d/status", pid);
    if ((fd = appview_open(buf, O_RDONLY)) == -1) {
        DBG(NULL);
        return -1;
    }

    if (appview_read(fd, buf, sizeof(buf)) == -1) {
        DBG(NULL);
        appview_close(fd);
        return -1;
    }

    if ((start = appview_strstr(buf, "VmSize")) == NULL) {
        DBG(NULL);
        appview_close(fd);
        return -1;        
    }
    
    entry = appview_strtok_r(start, delim, &last);
    entry = appview_strtok_r(NULL, delim, &last);
    if (entry == NULL) {
        DBG(NULL);
        appview_close(fd);
        return -1;        
    }
    
    if ((result = appview_strtol(entry, NULL, 0)) == (long)0) {
        DBG(NULL);
        appview_close(fd);
        return -1;
    }
    
    appview_close(fd);
    return (int)result;
}

int
osGetNumThreads(pid_t pid)
{
    int fd, i;
    long result;
    char *entry, *last;
    const char delim[] = " ";
    char buf[1024];

    // Get the size of the file with stat, malloc buf then appview_free
    appview_snprintf(buf, sizeof(buf), "/proc/%d/stat", pid);
    if ((fd = appview_open(buf, O_RDONLY)) == -1) {
        DBG(NULL);
        return -1;
    }

    if (appview_read(fd, buf, sizeof(buf)) == -1) {
        DBG(NULL);
        appview_close(fd);
        return -1;
    }

    entry = appview_strtok_r(buf, delim, &last);
    for (i = 1; i < 20; i++) {
        entry = appview_strtok_r(NULL, delim, &last);
    }
    appview_close(fd);

    if ((result = appview_strtol(entry, NULL, 0)) == (long)0) {
        DBG(NULL);
        return -1;
    }
    return (int)result;
}

int
osGetNumFds(pid_t pid)
{
    int nfile = 0;
    DIR *dirp;
    struct dirent *entry;
    char buf[1024];

    appview_snprintf(buf, sizeof(buf), "/proc/%d/fd", pid);
    if ((dirp = appview_opendir(buf)) == NULL) {
        DBG(NULL);
        return -1;
    }
    
    while ((entry = appview_readdir(dirp)) != NULL) {
        if (entry->d_type != DT_DIR) {
            nfile++;
        }
    }

    appview_closedir(dirp);
    return nfile - 1; // we opened one fd to read /fd :)
}

int
osGetNumChildProcs(pid_t pid)
{
    int nchild = 0;
    DIR *dirp;
    struct dirent * entry;
    char buf[1024];

    appview_snprintf(buf, sizeof(buf), "/proc/%d/task", pid);
    if ((dirp = appview_opendir(buf)) == NULL) {
        DBG(NULL);
        return -1;
    }
    
    while ((entry = appview_readdir(dirp)) != NULL) {
            nchild++;
    }

    appview_closedir(dirp);
    return nchild - 3; // Not including the parent proc itself and ., ..
}

int
osInitTimer(platform_time_t *cfg)
{
    int fd;
    uint64_t val;
    char *buf;
    const char path[] = "/proc/cpuinfo";

    if ((fd = appview_open(path, O_RDONLY)) == -1) {
        DBG(NULL);
        return -1;
    }
    
    /*
     * Anecdotal evidence that there is a max size to proc entrires.
     * In any case this should be big enough.
     */    
    if ((buf = appview_calloc(1, MAX_PROC)) == NULL) {
        DBG(NULL);
        appview_close(fd);
        return -1;
    }
    
    if (appview_read(fd, buf, MAX_PROC) == -1) {
        DBG(NULL);
        appview_close(fd);
        appview_free(buf);
        return -1;
    }

    if (appview_strstr(buf, "rdtscp") == NULL) {
        cfg->tsc_rdtscp = FALSE;
    } else {
        cfg->tsc_rdtscp = TRUE;
    }
    
    if (appview_strstr(buf, "tsc_reliable") == NULL) {
        cfg->tsc_invariant = FALSE;
    } else {
        cfg->tsc_invariant = TRUE;
    }

    if ((cfg->freq = getProcVal(buf, "cpu MHz")) == -1) {
        cfg->freq = -1;
    }

    if (((val = getProcVal(buf, "CPU architecture")) != -1) &&
        (val >= 8)) {
        cfg->gptimer_avail = TRUE;
    } else {
        cfg->gptimer_avail = FALSE;
    }

#ifdef __aarch64__
    /*
     * This uses the General Purpose Timer definition in an aarch64 instance.
     * The frequency is the lower 32 bits of the CNTFRQ_EL0 register and
     * is defined as HZ. The configured freq is defined in Mhz.
     */
    if (cfg->gptimer_avail == TRUE) {
        uint64_t freq;

        __asm__ volatile (
            "mrs x1, CNTFRQ_EL0 \n"
            "mov %0, x1  \n"
            : "=r" (freq)                // output
            :                            // inputs
            :                            // clobbered register
            );

        freq &= 0x0000000ffffffff;
        freq /= 1000000;
        cfg->freq = freq;
    }
#elif defined(__x86_64__)
    if ((cfg->tsc_invariant == TRUE) && (cfg->freq != -1)) {
        cfg->gptimer_avail = TRUE;
    } else {
        cfg->gptimer_avail = FALSE;
    }
#else
#error No architecture defined
#endif

    appview_close(fd);
    appview_free(buf);
    if (cfg->freq == (uint64_t)-1) {
        DBG(NULL);
        return -1;
    }
    return 0;
}

int
osIsFilePresent(const char *path)
{
    struct stat sb = {0};

    if (appview_stat(path, &sb) != 0) {
        return -1;
    } else {
        return sb.st_size;
    }
}

int
osGetCmdline(pid_t pid, char **cmd)
{
    int fd = -1;
    int bytesRead = 0;
    char path[64];

    if (!cmd) return 0;
    char *buf = *cmd;
    buf = NULL;


    if ((buf = appview_calloc(1, NCARGS)) == NULL) {
        goto out;
    }

    if (appview_snprintf(path, sizeof(path), "/proc/%d/cmdline", pid) < 0) {
        goto out;
    }

    if ((fd = appview_open(path, O_RDONLY)) == -1) {
        DBG(NULL);
        goto out;
    }

    if ((bytesRead = appview_read(fd, buf, NCARGS)) <= 0) {
        DBG(NULL);
        goto out;
    }

    // Replace all but the last null with spaces
    int i;
    for (i=0; i < (bytesRead - 1); i++) {
        if (buf[i] == '\0') buf[i] = ' ';
    }

out:
    if (!buf || !buf[0]) {
        if (buf) appview_free(buf);
        buf = appview_strdup("none");
    } else {
        // buf is big; try to appview_strdup what we've used and appview_free the rest
        char *tmp = appview_strdup(buf);
        if (tmp) {
            appview_free(buf);
            buf = tmp;
        }
    }

    if (fd != -1) appview_close(fd);
    *cmd = buf;
    return (*cmd != NULL);
}

int
osGetArgv(pid_t pid, char *buf, size_t blen)
{
    int i, fd = -1, argc = 0, bytesRead = 0;
    char path[64];

    if (!buf) return 0;

    if (appview_snprintf(path, sizeof(path), "/proc/%d/cmdline", pid) < 0) {
        goto out;
    }

    if ((fd = appview_open(path, O_RDONLY)) == -1) {
        DBG(NULL);
        goto out;
    }

    if ((bytesRead = appview_read(fd, buf, blen)) <= 0) {
        DBG(NULL);
        goto out;
    }

    // Replace all but the last null with spaces
    for (i=0; i < (bytesRead - 1); i++) {
        if (buf[i] == '\0') argc++;
    }

out:
    if (fd != -1) appview_close(fd);
    return argc;
}

/*
* Stops the "signal timer"
*/
bool
osTimerStop(void) {
    if (g_sigUsrState.timerId) {
        timer_delete(g_sigUsrState.timerId);
        g_sigUsrState.timerId = 0;
        return TRUE;
    }

    return FALSE;
}


/*
* Check if AppView Handler is enabled
*/
bool
osIsAppViewHandlerActive(void) {
    return g_sigUsrState.appviewHandlerActive;
}

/*
* Sets the application SIGUSR2 signal action
*/
void
osSetAppSigaction(const struct sigaction *act) {
    g_sigUsrState.appAction.sa_flags = act->sa_flags;
    if (g_sigUsrState.appAction.sa_flags & SA_SIGINFO) {
        g_sigUsrState.appAction.sa_sigaction = act->sa_sigaction;
    } else {
        g_sigUsrState.appAction.sa_handler = act->sa_handler;
    }
    g_sigUsrState.appAction.sa_mask = act->sa_mask;
}

/*
* Get the application SIGUSR2 signal action
*/
void
osGetAppSigaction(struct sigaction *const act) {
    *act = g_sigUsrState.appAction;
}

/*
* Call the Application handler
*
* Retun TRUE if the application handler was called, FALSE otherwise
*/
bool
osCallAppSigaction(int sig, siginfo_t *info, void *secret) {

    /*
    * Determine if signal info originates from AppView timer
    */
    if (!info) {
        goto callAppaction;
    }

    if (info->si_signo != SIGUSR2) {
        goto callAppaction;
    }

    if (info->si_code != SI_TIMER) {
        goto callAppaction;
    }

    if (info->si_int != SIG_ORIGIN_APPVIEW_ID) {
        goto callAppaction;
    }

    return FALSE;

callAppaction:
    if (g_sigUsrState.appAction.sa_flags & SA_SIGINFO) {
        g_sigUsrState.appAction.sa_sigaction(sig, info, secret);
    } else {
        g_sigUsrState.appAction.sa_handler(sig);
    }
    return TRUE;
}

/*
* Register the signal handler for Appappview and starts the signal timer.
* On timer expiration will generates SIGUSR2 signal
*/
bool
osThreadInit(void(*handler)(int, siginfo_t *, void *), unsigned interval) {
    struct sigaction sact;
    struct sigevent sevent = {0};
    struct itimerspec tspec;
    sigemptyset(&sact.sa_mask);
    sact.sa_handler = (void (*))handler;
    sact.sa_flags = SA_SIGINFO | SA_RESTART;

    if (!g_fn.sigaction) {
        return FALSE;
    }

    if (g_fn.sigaction(SIGUSR2, &sact, NULL) == -1) {
        DBG("errno %d", errno);
        return FALSE;
    }

    sevent.sigev_notify = SIGEV_SIGNAL;
    sevent.sigev_signo = SIGUSR2;
    sevent.sigev_value.sival_int = SIG_ORIGIN_APPVIEW_ID;
    g_sigUsrState.appviewHandlerActive = TRUE;

    if (timer_create(CLOCK_MONOTONIC, &sevent, &g_sigUsrState.timerId) == -1) {
        DBG("errno %d", errno);
        return FALSE;
    }

    tspec.it_interval.tv_sec = 0;
    tspec.it_interval.tv_nsec = 0;
    tspec.it_value.tv_sec = interval;
    tspec.it_value.tv_nsec = 0;
    if (timer_settime(g_sigUsrState.timerId, 0, &tspec, NULL) == -1) {
        DBG("errno %d", errno);
        return FALSE;
    }
    return TRUE;
}

// In linux, this is declared weak so it can be overridden by the strong
// definition in src/javaagent.c.  The appview library will do this.
// This weak definition allows us to not have to define this symbol for
// unit tests or for the appview executable.
void __attribute__((weak))
initJavaAgent(void) {
    return;
}

void
osInitJavaAgent(void)
{
    initJavaAgent();
}


/*
 * Example from /proc/self/maps:
 * 7f1b23bd4000-7f1b23bd7000 rw-p 001e3000 08:01 402063 /usr/lib/x86_64-linux-gnu/libc-2.29.so
 */
int
osGetPageProt(uint64_t addr)
{
    int prot = -1;
    size_t len = 0;
    char *buf = NULL;

    if (addr == 0) {
        return -1;
    }

    FILE *fstream = appview_fopen("/proc/self/maps", "r");
    if (fstream == NULL) return -1;

    while (appview_getline(&buf, &len, fstream) != -1) {
        char *end = NULL;
        uint64_t addr1 = appview_strtoull(buf, &end, 0x10);
        if ((addr1 == 0) || (addr1 == ULLONG_MAX)) {
            if (buf) appview_free(buf);
            appview_fclose(fstream);
            return -1;
        }

        uint64_t addr2 = appview_strtoull(end + 1, &end, 0x10);
        if ((addr2 == 0) || (addr2 == ULLONG_MAX)) {
            if (buf) appview_free(buf);
            appview_fclose(fstream);
            return -1;
        }

        appviewLog(CFG_LOG_TRACE, "addr 0x%lux addr1 0x%lux addr2 0x%lux\n", addr, addr1, addr2);

        if ((addr >= addr1) && (addr < addr2)) {
            char *perms = end + 1;
            appviewLog(CFG_LOG_DEBUG, "matched 0x%lx to 0x%lx-0x%lx\n\t%c%c%c", addr, addr1, addr2, perms[0], perms[1], perms[2]);
            prot = 0;
            prot |= perms[0] == 'r' ? PROT_READ : 0;
            prot |= perms[1] == 'w' ? PROT_WRITE : 0;
            prot |= perms[2] == 'x' ? PROT_EXEC : 0;
            if (buf) appview_free(buf);
            break;
        }

        if (buf) {
            appview_free(buf);
            buf = NULL;
        }

        len = 0;
    }

    appview_fclose(fstream);
    return prot;
}

int
osNeedsConnect(int fd)
{
    int rc, timeout;
    struct pollfd fds;

    timeout = 0;
    appview_memset(&fds, 0x0, sizeof(fds));
    fds.events = POLLRDHUP | POLLOUT;

    fds.fd = fd;

    rc = appview_poll(&fds, 1, timeout);

    if ((rc != 0) && (((fds.revents & POLLRDHUP) != 0) || ((fds.revents & POLLERR) != 0))) return 1;
    return 0;
}

/*
 * Example from /proc/<pid>/cgroup:
   2:freezer:/
   11:pids:/user.slice/user-1000.slice/user@1000.service
   10:memory:/user.slice/user-1000.slice/user@1000.service
   9:cpuset:/
   8:devices:/user.slice
   7:net_cls,net_prio:/
   6:blkio:/user.slice
   5:perf_event:/
   4:hugetlb:/
   3:cpu,cpuacct:/user.slice
   2:rdma:/
   1:name=systemd:/user.slice/user-1000.slice/user@1000.service/gnome-launched-emacs.desktop-21457.appview
   0::/user.slice/user-1000.slice/user@1000.service/gnome-launched-emacs.desktop-21457.appview
 */
bool
osGetCgroup(pid_t pid, char *cgroup, size_t cglen)
{
    size_t len = 0;
    char *buf = NULL;
    char path[PATH_MAX];

    if (cglen <= 0) {
        return FALSE;
    }

    if (appview_snprintf(path, sizeof(path), "/proc/%d/cgroup", pid) < 0) return FALSE;

    FILE *fstream = appview_fopen(path, "r");
    if (fstream == NULL) return FALSE;

    while (appview_getline(&buf, &len, fstream) != -1) {
        if (buf && appview_strstr(buf, "0::")) {
            appview_strncpy(cgroup, buf, cglen);
            char *nonl = appview_strchr(cgroup, '\n');
            if (nonl) *nonl = '\0';

            appview_free(buf);
            appview_fclose(fstream);
            return TRUE;
        }

        if (buf) appview_free(buf);
        buf = NULL;
        len = 0;
    }

    appview_fclose(fstream);
    return FALSE;
}

char *
osGetFileMode(mode_t perm)
{
    char *mode = appview_malloc(MODE_STR);
    if (!mode) return NULL;

    mode[0] = (perm & S_IRUSR) ? 'r' : '-';
    mode[1] = (perm & S_IWUSR) ? 'w' : '-';
    mode[2] = (perm & S_IXUSR) ? 'x' : '-';
    mode[3] = (perm & S_IRGRP) ? 'r' : '-';
    mode[4] = (perm & S_IWGRP) ? 'w' : '-';
    mode[5] = (perm & S_IXGRP) ? 'x' : '-';
    mode[6] = (perm & S_IROTH) ? 'r' : '-';
    mode[7] = (perm & S_IWOTH) ? 'w' : '-';
    mode[8] = (perm & S_IXOTH) ? 'x' : '-';
    mode[9] = '\0';
    return mode;
}

char *
osGetUserName(unsigned uid)
{
    struct passwd pwd;
    struct passwd *pwd_res = NULL;
    char buf[4096];

    int ret = appview_getpwuid_r(uid, &pwd, buf, sizeof(buf), &pwd_res);
    if (ret)
        return NULL;
    return appview_strdup(pwd.pw_name);
}

char *
osGetGroupName(unsigned gid)
{
    struct group grp;
    struct group *grp_res = NULL;
    char buf[4096];

    appview_getgrgid_r(gid, &grp, buf, sizeof(buf), &grp_res);
    if (grp_res)
        return appview_strdup(grp.gr_name);
    return NULL;
}

// Return process specific CPU usage in microseconds
long long
osGetProcCPU(void) {
    struct rusage ruse;
    
    if (appview_getrusage(RUSAGE_SELF, &ruse) != 0) {
        return -1;
    }

    return
        (((long long)ruse.ru_utime.tv_sec + (long long)ruse.ru_stime.tv_sec) * 1000 * 1000) +
        ((long long)ruse.ru_utime.tv_usec + (long long)ruse.ru_stime.tv_usec);
}

uint64_t
osFindLibrary(const char *library, pid_t pid, bool matchLibraryExactly)
{
    char filename[PATH_MAX];
    char line[1024];
    FILE *fd;
    uint64_t addr = 0;

    appview_snprintf(filename, sizeof(filename), "/proc/%d/maps", pid);
    if ((fd = appview_fopen(filename, "r")) == NULL) {
        // return no proc found as opposed to no libappview found
        return -1;
    }

    if (matchLibraryExactly) { // pathFromLine string == library string
        char pathFromLine[512];
        while(appview_fgets(line, sizeof(line), fd)) {
            //7ff775e88000-7ff77606f000 r-xp 00000000 ca:01 17348 /lib/x86_64-linux-gnu/libc-2.27.so
            if ((appview_sscanf(line, "%*x-%*x %*s %*x %*s %*d %512s", pathFromLine) == 1)
                && !appview_strcmp(pathFromLine, library)) {
                addr = appview_strtoull(line, NULL, 16);
                break;
            }
        }
    } else {                   // line *contains* library string
        while(appview_fgets(line, sizeof(line), fd)) {
            if (appview_strstr(line, library)) {
                addr = appview_strtoull(line, NULL, 16);
                break;
            }
        }
    }

    appview_fclose(fd);
    return addr;
}

void
osCreateSM(proc_id_t *proc, unsigned long addr)
{
    export_sm_t *exaddr;

    if (!proc) return;

    // Create the anonymous sm
    if ((proc->smfd = appview_memfd_create(SM_NAME, MFD_ALLOW_SEALING | MFD_CLOEXEC)) == -1) {
        proc->smfd = -1;
        return;
    }

    // size is initially 0 and needs to be increased
    if (appview_ftruncate(proc->smfd, sizeof(export_sm_t)) == -1) {
        appview_close(proc->smfd);
        return;
    }

    if ((exaddr = appview_mmap(NULL, sizeof(export_sm_t), PROT_READ | PROT_WRITE,
                             MAP_SHARED, proc->smfd, 0)) == MAP_FAILED) {
        appview_close(proc->smfd);
        return;
    }

    proc->smaddr = addr;          // these are not currently used, useful for debug
    exaddr->viewed = FALSE;
    exaddr->cmdAttachAddr = addr; // this is the primary update

    /*
     * We need to unmap before setting the segment read only with seals
     * A close here will remove the link in /proc/<pid>/fd/FD
     * We will close when we reattach
     */
    appview_munmap(exaddr, sizeof(export_sm_t));   // on error, what?

    /*
     * Make this read-only for other procs
     * It would be nice to use F_SEAL_FUTURE_WRITE here, but it is
     * available on >= 5.1 kernels and we support older versions.
     */
    if (appview_fcntl(proc->smfd, F_ADD_SEALS, F_SEAL_WRITE | F_SEAL_GROW) == -1) {
        appview_close(proc->smfd);
    }
}

/*
 * Get the file descriptor from the filename for a given pid
 */
int
osFindFd(pid_t pid, const char *fname)
{
    int fd = -1;
    DIR *dirp;
    char *cwd;
    struct dirent *entry;
    char buf[PATH_MAX], link[256];

    if ((cwd = appview_getcwd(NULL, 0)) == NULL) {
        DBG(NULL);
        return -1;
    }

    appview_snprintf(buf, sizeof(buf), "/proc/%d/fd", pid);

    if (appview_chdir(buf) == -1) {
        appview_free(cwd);
        DBG(NULL);
        return -1;
    }

    if ((dirp = appview_opendir(buf)) == NULL) {
        appview_chdir(cwd);
        appview_free(cwd);
        DBG(NULL);
        return -1;
    }

    while ((entry = appview_readdir(dirp)) != NULL) {
        if (entry->d_type != DT_DIR) {
                if (appview_readlink(entry->d_name, link, sizeof(link)) == -1) {
                appviewLogError("%s: can't get path to %s", __FUNCTION__, entry->d_name);
                break;
            }

            if (appview_strstr(link, fname)) {
                fd = appview_strtol(entry->d_name, NULL, 0);
                if ((fd == LONG_MIN) || (fd == LONG_MAX) || (fd == 0)) fd = -1;
                break;
            }
        }
    }

    appview_chdir(cwd);
    appview_closedir(dirp);
    appview_free(cwd);
    return fd;
}

/*
 * Change protection for specified memory region using 
 * specified combintation flags and extraflags.
 * Returns TRUE in case of operation success, FALSE otherwise
 * Note: in case of success the `osMemPermRestore` should be called later
 * with flags argument
 */
bool
osMemPermAllow(void *addr, size_t len, int flags, int extraflags) {
    return appview_mprotect(addr, len, flags | extraflags) == 0;
}

/*
 * Restore permission for specified memory region.
 * Returns TRUE in case of operation success, FALSE otherwise
 * Note: This function should be called in case of success of `osMemPermAllow`
 */
bool
osMemPermRestore(void *addr, size_t len, int flags) {
    return appview_mprotect(addr, len, flags) == 0;
}

/*
 * Retrieve base address for the process
 * Returns TRUE in case of operation success, FALSE otherwise
 */
bool
osGetBaseAddr(uint64_t *addr) {
    uint64_t base_addr = 0;
    char perms[5];
    char offset[20];
    char buf[1024];
    char pname[1024];
    FILE *fp;

    if (osGetProcname(pname, sizeof(pname)) == -1) {
        return FALSE;
    }

    if ((fp = appview_fopen("/proc/self/maps", "r")) == NULL) {
        return FALSE;
    }

    while (appview_fgets(buf, sizeof(buf), fp) != NULL) {
        uint64_t addr_start;
        appview_sscanf(buf, "%lx-%*x %s %*s %s %*d", &addr_start, perms, offset);
        if (appview_strstr(buf, pname) != NULL) {
            base_addr = addr_start;
            break;
        }
    }

    appview_fclose(fp);
    if (base_addr) {
        *addr = base_addr;
        return TRUE;
    }
    return FALSE;
}

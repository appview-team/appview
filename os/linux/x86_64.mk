...
#ARCH_COPY=cp ./lib/$(OS)/$(ARCH)/libappview.so ./lib/$(OS)/libappview.so && \
#	objcopy -I binary -O elf64-x86-64 -B i386 ./lib/$(OS)/libappview.so ./lib/$(OS)/libappview.o && \
#	rm -f ./lib/$(OS)/libappview.so

$(LIBAPPVIEW): src/wrap.c src/state.c src/httpstate.c src/metriccapture.c src/report.c src/httpagg.c src/httpmatch.c src/plattime.c src/fn.c os/$(OS)/os.c src/cfgutils.c src/cfg.c src/transport.c src/backoff.c src/log.c src/mtc.c src/circbuf.c src/linklist.c src/evtformat.c src/ctl.c src/mtcformat.c src/com.c src/appviewstdlib.c src/dbg.c src/strsearch.c src/sysexec.c src/gocontext.S src/appviewelf.c src/oci.c src/wrap_go.c src/utils.c src/strset.c src/javabci.c src/javaagent.c src/ipc.c src/ipc_resp.c src/snapshot.c src/coredump.c src/evtutils.c src/notify.c
	...
	$(CC) -c $(LIBRARY_CFLAGS) $(ARCH_CFLAGS) $(YAML_DEFINES) $(INCLUDES) $^

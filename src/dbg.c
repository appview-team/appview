...
dbgInit(void)
{
    ...
    g_dbg = appview_calloc(1, sizeof(*g_dbg));
}
...
updateLine(line_t *line, char *str)
{
    ...
    line->instance[i].err = appview_errno;

    // The atomic swap allows us to appview_free previous strings without leaking
    // memory and equally importantly, ensures we can't double-appview_free a str.
    char *prev_str = atomicSwapString(&line->instance[i].str, str);
    if (prev_str) appview_free(prev_str);
}
...
resetLine(line_t *line)
{
    ...
    appview_memset(line, 0, sizeof(*line));
}
...
dbgCountMatchingLines(const char *str)
{
    ...
    for (i=0; i < MAX_NUM_LINES; i++) {
        if (!g_dbg->lines[i].key) break;
        if (appview_strstr(g_dbg->lines[i].key, str)) {
            result++;
        }
    }
    return result;
...
dbgOutputHeaderLine(FILE *f)
{
    ...
    struct tm t;
    char buf[128] = {0};
    appview_strftime(buf, sizeof(buf), "%FT%TZ", appview_gmtime_r(&tv.tv_sec, &t));
    appview_fprintf(f, "AppView Version: %s   Dump From: %s\n", APPVIEW_VER, buf);
}
...
dbgDumpAll(FILE *f)
{
    ...
    for (i=0; i < MAX_NUM_LINES; i++) {
        ...
        for (j=0; j<MAX_INSTANCES_PER_LINE; j++) {
            ...
            char t_str[64];
            if (!o->time) continue;
            if (!appview_gmtime_r(&o->time, &t)) continue;
            if (!appview_strftime(t_str, sizeof(t_str), "%s", &t)) continue;

            if (j==0) {
                appview_fprintf(f, "%lu: %s %s %d(%s) %s\n",
                    l->count, l->key,
                    t_str, o->err, appview_strerror(o->err), o->str);
            } else {
                appview_fprintf(f, "    %s %d(%s) %s\n",
                    t_str, o->err, appview_strerror(o->err), o->str);
            }
        }
    }
...
search_for_spot:
    ...
    for (i=0; i < MAX_NUM_LINES; i++) {
        ...
        // We don't have to do a appview_strcmp; comparing the pointers works
...
dbgAddLine(const char *key, const char *fmt, ...)
{
    ...
    if (fmt) {
        va_list args;
        va_start(args, fmt);
        int rv = appview_vasprintf(&str, fmt, args);
        va_end(args);
        if (rv == -1) {
            if (str) appview_free(str);
            str = NULL;
...
// definition in src/wrap.c.  The appview library will do this.
...
// appviewLogDebug and appviewLogTrace when DEBUG is not defined.  This warning
// can happen during compilation of calling code when function arguments are
// variables.
void
appviewLogDropItOnTheFloor(const char *format, ...)
{
    return;
}

void
appviewLogHex(cfg_log_level_t level, const void *data, size_t size, const char *format, ...)
{
    ...
    int buflen = appview_vsnprintf(buf, sizeof(buf), format, args);
    va_end(args);
    if (buflen == -1) {
        appviewLog(level, "%s (format too long!) (%ld bytes)", format, size);
    } else {
        appviewLog(level, "%s (%ld bytes)", buf, size);
    }
    ...
    for (i = 0; i < size; ++i) {
        if (i && (i % 16) == 0) {
            appviewLog(level, "  %04x: %s %s", i-16, hex, txt);
            hex[0] = '\0'; txt[0] = '\0'; phex = hex; // reset
        }

        appview_sprintf(phex, " %02x", pdata[i]); phex += 3;
        if ((i % 8) == 7) {
            *phex++ = ' '; *phex = '\0';
        }

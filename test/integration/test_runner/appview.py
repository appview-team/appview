...
class DataListener(abc.ABC):
    ...
    def __init__(self, collector: AppViewDataCollector, host: str, port: int) -> None:
        self.port = port
        self.host = host
...
class AppViewUDPDataListener(DataListener):
    __server = None
    ...
    def create_server(self) -> socketserver.ThreadingUDPServer:
        ...
        logging.info(f"Starting appview {self.name} listener at {self.host}:{self.port}")
    ...
    def stop_server(self) -> None:
         logging.info(f"Stopping appview {self.name} listener.")
         self.__server.shutdown()

class AppViewTCPDataListener(DataListener):
    server = None
...
def create_listener(type: str, collector: AppViewDataCollector) -> DataListener:
    default_listeners = {"udp": AppViewUDPDataListener, "tcp": AppViewTCPDataListener}

...
get_node(char *events, char *start, char *tag)
{
    ...
    if (((thisval = strstr(events, start)) != NULL) &&
        ((thisval = strstr(thisval, tag)) != NULL) &&
        ((value = index(thisval, ':')) != NULL) && 
        ((endval = index(value, ',')) != NULL)) {
        *endval = '\0';
...
client_child() {
    ...
    if (verbose > 0) fprintf(stdout, "Child starting\n");
